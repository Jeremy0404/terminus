...
describe('GhReleaseTarget', () => {
  it('reads the pending release, the latest one and the last release run', () => {
    ...
    const gh = fakeGh({
      'run view': '{"jobs":[{"name":"deploy","conclusion":"success"}]}',
    ...
    expect(new GhReleaseTarget(gh).state(repo)).toEqual({
      ...
      lastRun: { deploymentVerified: true, id: 31, version: '0.2.0', state: 'succeeded', startedAt: '2026-08-10T16:54:10Z', url: 'https://github.com/o/r/actions/runs/31' },
    });
  });
