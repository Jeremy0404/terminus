export type FailureKind = 'check-failed' | 'loop-detected' | 'budget-exceeded' | 'agent-crashed' | 'quota-exhausted' | 'publish-failed' | 'interrupted' | 'isolation-breach' | 'merge-conflict' | 'sync-failed';
...
const NEVER_RETRIED: ReadonlySet<FailureKind> = new Set(['quota-exhausted', 'interrupted', 'isolation-breach']);
...
export function decideAfterFailure(failuresInPhase: readonly Failure[], policy: FailurePolicy): 'retry' | 'block' {
  ...
  if (!latest || NEVER_RETRIED.has(latest.kind)) return 'block';
