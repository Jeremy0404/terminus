...
export const api = {
  ...
  release: (appId: string, fresh = false) => request<ReleaseStateDto | null>(`/apps/${appId}/release${fresh ? '?fresh=1' : ''}`),
  deploy: (appId: string, version: string) => post<DeploymentDto>(`/apps/${appId}/release/deploy`, { version }),
