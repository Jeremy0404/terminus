...
export function DecisionDossier({ detail, network }: { detail: TaskDetailDto; network: NetworkDto }) {
  ...
  const effect = status.kind === 'awaiting-decision' ? 'answer' : status.kind === 'awaiting-gate' && status.gate === 'merge' ? 'merge' : phase === 'brief' ? 'brief' : phase === 'architecture' ? 'architecture' : status.kind === 'awaiting-gate' && status.gate === 'plan-approval' ? 'plan' : 'approve';
  ...
  return <section className="decision-dossier">
    ...
    {(awaiting || docs.length > 0) && <details className="dossier-documents"><summary>{t('dossier.documents', { count: docs.length })}</summary>
      {docs.length === 0 && <p className="muted">{t('dossier.noDocuments')}</p>}
      {docs.map((doc) => <details key={doc.name}><summary>{t(doc.name === 'spec.md' ? 'dossier.spec' : 'dossier.plan')}</summary><pre className="brief-preview">{doc.content}</pre>{doc.truncated && <p className="muted">{t('dossier.truncated')}</p>}</details>)}
    </details>}
